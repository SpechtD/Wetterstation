...
;Da der �C C8051F340 viel schneller getaktet ist, als das LCD-Display es verarbeiten kann,
...
#include <intrins.h>						// f�r _crol_ -Funktion
#include <stdio.h>							//	f�r sprintf


unsigned char LCD_PORT;
#define LCD_PORT P3 						//	LCD-Display an P3 im Port-4-Bit-Modus
...
/******************************************************************************************
 *  Zeitverz�gerung:  wartet ca.  anzahl mal 100�s
 ******************************************************************************************/
...
void LCDbefehl (unsigned char befehl)
	...
	a1 = (a1 & 0x0F) | 0x10;	//	4 Bits maskieren, �bergabetakt = 1, High-Nibbel senden
	LCD_PORT = a1;
	LCD_PORT = a1 & 0x00;		// Takt = 0
	a1 = befehl;					// Low-Nibbel
	a1 = (a1 & 0x0F) | 0x10;	//	4 Bits maskieren, �bergabetakt = 1
	LCD_PORT = a1;
	LCD_PORT = a1 & 0x00;		// Takt = 0
...
//	wenn Busy nicht geht, Zeitverz�gerung 1ms verwenden 	**********************************
//	warte100u (10);
...
*/
/******************************************************************************************
 * Initialisierung des LCD- Displays f�r den 4-Bit-Modus
 ******************************************************************************************/
void initlcd (void)
	{
...
//	al >> 4;
	...
	a1 = (a1 & 0x0F) | 0x90;	//	4 Bits maskieren, �bergabetakt = 1, High-Nibbel senden
	LCD_PORT = a1;
	LCD_PORT = a1 & 0x80;		// Takt = 0
	a1 = zeichen;					// Low-Nibbel
	a1 = (a1 & 0x0F) | 0x90;	//	4 Bits maskieren, �bergabetakt = 1
	LCD_PORT = a1;
	LCD_PORT = a1 & 0x80;		// Takt = 0
...
void cursorpos (unsigned char position)
	{ 	LCDbefehl ( position | 0x80 ); }	// Kennung f�r DD RAM address set
